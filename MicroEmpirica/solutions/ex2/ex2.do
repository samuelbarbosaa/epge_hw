...
gen missing_renda = 0
replace missing_renda = 1 if renda == .

gen ind_renda = 0
replace ind_renda = 1 if renda > 0

egen id_dom = concat(v0102 v0103)
bysort id_dom: egen n_filhos = total(cond_dom==3 & idade<18)
replace n_filhos = 0 if cond_dom >= 3	

gen pia = 0
replace pia = 1 if idade >= 18 & idade <= 65

gen sp = 0
replace sp = 1 if pia==1 & missing_renda==0
...
keep v0101 v0102 v0103 mulher idade idade2 cond_dom casal anos_estudo ///
     pia n_filhos ind_renda renda log_renda  ///
	 peso estrato psu id_dom sp
...
svy, subpop(sp): reg log_renda anos_estudo mulher idade idade2
...
svy, subpop(pia): probit ind_renda anos_estudo mulher casal mulher##casal n_filhos c.n_filhos##mulher
...
svy, subpop(sp): heckman log_renda anos_estudo mulher idade idade2, select(ind_renda = anos_estudo mulher casal mulher##casal n_filhos c.n_filhos##mulher)
