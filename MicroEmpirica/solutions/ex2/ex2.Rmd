...
pnad_df = 
  ...
  select(v0101, v0102, v0103, v0402, v0403, 
         ...
         v4011, v4729, v4617, v4618)

pnad_df = 
  rename(pnad_df,
     cond_familia = v0402,
     renda = v4718, anos_estudo = v4803, idade = v8005,
     sexo = v0302, cor_raca = v0404, estado_civil = v4011,
     peso = v4729, estrato = v4617, psu = v4618)

pnad_df = 
  pnad_df %>% 
  mutate(id_familia = paste0(v0101, v0102, v0103, v0403))

pnad_df = 
  pnad_df %>% 
  group_by(id_familia) %>% 
  mutate(n_filhos = sum(cond_familia == 3 & idade < 18)) %>% 
  ungroup()

pnad_df = 
  pnad_df %>% 
  mutate(n_filhos = replace(n_filhos, cond_familia >= 3, 0))
...
pnad_df = 
  pnad_df %>% 
  mutate(casado = ifelse(estado_civil == 1, 1, 0),
...
pnad_df = 
  mutate_at(pnad_df, vars(sexo, cor_raca, casado), as.factor)
...
save(pnad_df, file = "data/pnad/pnad2015_ex2.Rdata")
write_csv(pnad_df, "data/pnad/pnad2015_ex2.csv")
...
pnad_design = subset(pnad_design, PIA == 1 & !is.na(renda))
...
```{r}
subdesign = subset(pnad_design, renda != 0 & !is.na(renda))
...
heckit = selection(ind_renda ~ anos_estudo + casado*sexo + n_filhos*sexo,
          log_renda ~ anos_estudo + sexo + idade + idade2,
          data = pnad_design$variables,
          weights = pnad_design$variables$peso,
          method = "2step")
summary(heckit)
```
